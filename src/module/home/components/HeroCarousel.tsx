...
function HeroCarousel() {
  ...
  const buttonsAnimationClass =
    slideDirection === "from-right"
      ? "home-carousel-buttons-from-right"
      : "home-carousel-buttons-from-left";
  ...
  const slideContent = (
    <div className={activeSlide.contentClass}>
      ...
      <p
        className={`mt-5 text-base leading-[1.75] max-[640px]:text-[15px] ${activeSlide.descriptionClass} ${copyAnimationClass}`}
      >
        {activeSlide.description}
      </p>
      <div
        className={`mt-7 flex flex-wrap gap-3 ${isCentered ? "justify-center" : ""} ${buttonsAnimationClass}`}
      >
        <Link
          to={activeSlide.path}
  ...
  const slideImage = (
    <img
      className={`${activeSlide.imageClass} ${imageAnimationClass} block max-h-[360px] object-contain max-[860px]:max-h-[330px] max-[560px]:max-h-[280px]`}
  ...
  return (
    ...
    >
      <div
        className={`relative h-[620px] overflow-hidden max-[860px]:h-[640px] max-[560px]:h-[680px] ${activeSlide.backgroundClass}`}
      >
        <button
          type="button"
          className="absolute left-6 top-1/2 z-10 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full hover:bg-white/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-white max-[760px]:left-3"
        ...
        <button
          type="button"
          className="absolute right-6 top-1/2 z-10 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full hover:bg-white/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-white max-[760px]:right-3"
        ...
        >
          <div
            key={`${activeSlide.id}-${slideDirection}`}
