                ...
                <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    {values.map((val) => {
                        ...
                        return (
                            <div
                                key={val.title}
                                className={`flex flex-col items-center rounded-2xl p-8 text-center border ${val.cardBg} ${val.borderColor} transition-all duration-300 hover:-translate-y-1 hover:shadow-lg`}
                            >
                                <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-white shadow-md">
                                    <Icon className={`h-8 w-8 ${val.iconColor}`} />
                                </div>
                                <h3 className="text-xl font-extrabold text-ag-dark">
                                    {val.title}
                                </h3>
